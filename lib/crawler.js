...
Crawler.prototype.init = function init (options) {
	...
	log          : self.options.logger && function(){self.options.logger.log(arguments[1],arguments[0]);},
...
Crawler.prototype._release = function _release (options) {
	...
	if(options.debug){
	    logger.info("Releasing resource, %s",options.limiter);
	}
...
Crawler.prototype._pushToQueue = function _pushToQueue (options) {
	...
	if(options.debug){
	    logger.info("Acquired resource, %s",options.limiter);
	}
	...
	if(options.debug){
	    logger.info("Called by bottleneck, %s",options.limiter);
	}
